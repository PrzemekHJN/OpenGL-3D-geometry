...
/**
 * Some objects in 3D. The arrow keys can be used to rotate the object.
 * The number keys 1 through 6 select the object. The space bar toggles the use of anaglyph stereo.
 */

class Lab5 extends GLJPanel implements GLEventListener, KeyListener
{
    /**
     * A main routine to create and show a window that contains a panel of type Lab5.
    ...
    // --------------------------------------------------------------------------------------------------
    // drawing implemented for 1 and 2
    ...
    /**
     * The method that draws the current object, with it's modeling transformation.
     */
    ...
    // --------------------------------------------------------------------------------------------------
    // Draw the currently selected object, number 1, 2, 3, 4, 5, or 6.
    // (Objects should lie in the cube with x, y, and z coordinates in the range -5 to 5.)

        if (objectNumber == 1)
        { //corkscrew
            Corkscrew(gl2,1,8);
        }
        else if (objectNumber == 2){ //pyramid
            Pyramid(gl2,5,7); // object, size of object, number of triangles
        }
    }
    public void Corkscrew(GL2 gl2, int radius, int numberOfTurns)
    {
    ...
    public void Pyramid(GL2 gl2, float size, float n)
    {
        ...
        gl2.glRotatef(90, 30, 90, 5);
        gl2.glTranslatef(0, 0, -1f);
        for (int i = 1; i <= n; i++)
        {
            gl2.glColor3f(100, 0+i, 50); // setting colour to Pink
